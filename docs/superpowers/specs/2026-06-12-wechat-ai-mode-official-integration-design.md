...
把 AI Mode 作为现有小程序能力的扩展直接接入 `miniprogram/`，使该目录成为唯一、
可直接导入和校验的小程序项目根目录。

完成后必须满足：

1. `miniprogram/` 同时包含 `project.config.json`、`app.json`、
   `AGENTS.md`、`page-meta.json` 和 `skills/`。
2. `miniprogram/app.json` 直接声明 `agent.skills`、`agent.instruction`
   和 `agent.pageMetadata`。
3. `skills/craft-beer-guide` 是 `app.json` 声明的独立分包，路径与磁盘目录完全一致。
4. 原有浏览、择饮、学院、收藏、搜索和详情页继续工作，AI 是这些能力的新增入口，
   不是第二套独立产品。
5. 官方 validator 能以 `miniprogram/` 为唯一参数发现并校验 Skill。
...
开发者工具、官方 validator、预览和上传都以 `miniprogram/` 为项目路径。
仓库根目录不再作为微信开发者工具项目根目录。

根目录现有 `project.config.json` 和 `project.private.config.json` 迁移到
`miniprogram/`。迁移后的 `project.config.json` 不再设置
`miniprogramRoot: "miniprogram/"`，因为配置文件所在目录本身就是小程序根目录。

`packOptions.include` 显式包含 `skills`，防止上传或预览打包时忽略 Skill 目录。

## App 配置

`miniprogram/app.json` 在现有页面、Tab 和普通分包声明上原位扩展：
...
现有 `ai-mode/skill/craft-beer-guide/` 迁移到
`miniprogram/skills/craft-beer-guide/`。现有 Control/Candidate 实验文件保留，
...
- `miniprogram/skills/craft-beer-guide/data/catalog.js`
- `miniprogram/aiDetail/data/catalog.js`
...
Control/Candidate 实验构建可以继续生成独立测试快照，但这些快照不是微信项目根目录，
也不能取代 `miniprogram/` 的官方校验。

## 页面元数据与全局指令

`miniprogram/AGENTS.md` 是 `agent.instruction` 指向的全局行为边界，负责区分：
...
`miniprogram/page-meta.json` 描述现有普通页面及其 query 约束，用于文本链和页面理解。
...
- `miniprogram/project.config.json` 与 `miniprogram/app.json` 同级。
- `miniprogram/app.json` 存在正确的 `agent` 声明。
...
```powershell
node <official-validator>/scripts/validate.mjs miniprogram
```

通过标准不是“仓库自定义检查返回 0”，而是官方 validator 能从
`miniprogram/app.json` 自动发现 `skills/craft-beer-guide` 并完成检查。

### 开发者工具验证

使用已安装的微信开发者工具 CLI 对 `miniprogram/` 执行编译或预览探测，并记录：
...
1. 生成 AI 数据和知识库。
2. 运行全部仓库测试。
3. 以 `miniprogram/` 运行官方 validator。
4. 以 `miniprogram/` 导入或调用微信开发者工具。
...
`miniprogram/skills/craft-beer-guide` 被发现和上传。

## 验收标准

实现完成必须同时满足：

1. 官方 validator 以 `miniprogram/` 为根目录运行，不再出现缺少 `app.json`、
   `project.config.json` 或 Skill 目录的错误。
2. 微信开发者工具可直接导入 `miniprogram/` 并完成项目编译；若受 AppID 权限阻塞，
   有明确的工具输出和版本记录。
