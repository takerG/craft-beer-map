...
function bin() {
	var value = identity$4, domain = extent, threshold = thresholdSturges;
	...
	histogram.thresholds = function(_) {
		return arguments.length ? (threshold = typeof _ === "function" ? _ : constant(Array.isArray(_) ? slice$3.call(_) : _), histogram) : threshold;
	};
	return histogram;
}
//#endregion
...
function axis(orient, scale) {
	...
	function axis(context) {
		var values = tickValues == null ? scale.ticks ? scale.ticks.apply(scale, tickArguments) : scale.domain() : tickValues, format = tickFormat == null ? scale.tickFormat ? scale.tickFormat.apply(scale, tickArguments) : identity_default$4 : tickFormat, spacing = Math.max(tickSizeInner, 0) + tickPadding, range = scale.range(), range0 = +range[0] + offset, range1 = +range[range.length - 1] + offset, position = (scale.bandwidth ? center : number$2)(scale.copy(), offset), selection = context.selection ? context.selection() : context, path = selection.selectAll(".domain").data([null]), tick = selection.selectAll(".tick").data(values, scale).order(), tickExit = tick.exit(), tickEnter = tick.enter().append("g").attr("class", "tick"), line = tick.select("line"), text = tick.select("text");
...
function constant_default$10(x) {
	return function() {
		return x;
	};
}
...
function drag_default() {
	...
	drag.filter = function(_) {
		return arguments.length ? (filter = typeof _ === "function" ? _ : constant_default$9(!!_), drag) : filter;
	};
...
function hue(a, b) {
	var d = b - a;
	return d ? linear$2(a, d > 180 || d < -180 ? d - 360 * Math.round(d / 360) : d) : constant_default$8(isNaN(a) ? b : a);
}
function gamma(y) {
	return (y = +y) === 1 ? nogamma : function(a, b) {
		return b - a ? exponential(a, b, y) : constant_default$8(isNaN(a) ? b : a);
	};
}
function nogamma(a, b) {
	var d = b - a;
	return d ? linear$2(a, d) : constant_default$8(isNaN(a) ? b : a);
}
//#endregion
...
function value_default(a, b) {
	var t = typeof b, c;
	return b == null || t === "boolean" ? constant_default$8(b) : (t === "number" ? number_default : t === "string" ? (c = color(b)) ? (b = c, rgb_default) : string_default : b instanceof color ? rgb_default : b instanceof Date ? date_default : isNumberArray(b) ? numberArray_default : Array.isArray(b) ? genericArray : typeof b.valueOf !== "function" && typeof b.toString !== "function" || isNaN(b) ? object_default : number_default)(a, b);
}
//#endregion
...
function parseCss(value) {
	...
	return m.isIdentity ? identity$3 : decompose_default(m.a, m.b, m.c, m.d, m.e, m.f);
}
...
function brush(dim) {
	...
	function started(event) {
		...
		if (type === "overlay") {
			...
			state.selection = selection = [[w0 = dim === Y ? W : min$2(pts[0][0], pts[1][0]), n0 = dim === X ? N : min$2(pts[0][1], pts[1][1])], [e0 = dim === Y ? E : max$3(pts[0][0], pts[1][0]), s0 = dim === X ? S : max$3(pts[0][1], pts[1][1])]];
		...
		function moved(event) {
			...
			if (shifting && !lockX && !lockY && points.length === 1) {
				...
				if (abs$3(point.cur[0] - point[0]) > abs$3(point.cur[1] - point[1])) lockY = true;
				else lockX = true;
			}
	...
	brush.extent = function(_) {
		return arguments.length ? (extent = typeof _ === "function" ? _ : constant_default$7(number2(_)), brush) : extent;
	};
	brush.filter = function(_) {
		return arguments.length ? (filter = typeof _ === "function" ? _ : constant_default$7(!!_), brush) : filter;
	};
	brush.touchable = function(_) {
		return arguments.length ? (touchable = typeof _ === "function" ? _ : constant_default$7(!!_), brush) : touchable;
	};
...
var pi$2 = Math.PI, tau$3 = 2 * pi$2, epsilon$4 = 1e-6, tauEpsilon = tau$3 - epsilon$4;
...
var Path = class {
	...
	arcTo(x1, y1, x2, y2, r) {
		...
		if (this._x1 === null) this._append`M${this._x1 = x1},${this._y1 = y1}`;
		...
		else if (!(Math.abs(y01 * x21 - y21 * x01) > epsilon$4) || !r) this._append`L${this._x1 = x1},${this._y1 = y1}`;
		else {
			...
			if (Math.abs(t01 - 1) > epsilon$4) this._append`L${x1 + t01 * x01},${y1 + t01 * y01}`;
	...
	arc(x, y, r, a0, a1, ccw) {
		...
		else if (Math.abs(this._x1 - x0) > epsilon$4 || Math.abs(this._y1 - y0) > epsilon$4) this._append`L${x0},${y0}`;
		if (!r) return;
		if (da < 0) da = da % tau$3 + tau$3;
		if (da > tauEpsilon) this._append`A${r},${r},0,1,${cw},${x - dx},${y - dy}A${r},${r},0,1,${cw},${this._x1 = x0},${this._y1 = y0}`;
		else if (da > epsilon$4) this._append`A${r},${r},0,${+(da >= pi$2)},${cw},${this._x1 = x + r * Math.cos(a1)},${this._y1 = y + r * Math.sin(a1)}`;
	}
	rect(x, y, w, h) {
...
function ribbon(headRadius) {
	...
	function ribbon() {
		...
		if (ap > 1e-12) {
			if (abs$2(sa1 - sa0) > ap * 2 + 1e-12) sa1 > sa0 ? (sa0 += ap, sa1 -= ap) : (sa0 -= ap, sa1 += ap);
			else sa0 = sa1 = (sa0 + sa1) / 2;
			if (abs$2(ta1 - ta0) > ap * 2 + 1e-12) ta1 > ta0 ? (ta0 += ap, ta1 -= ap) : (ta0 -= ap, ta1 += ap);
	...
	if (headRadius) ribbon.headRadius = function(_) {
		return arguments.length ? (headRadius = typeof _ === "function" ? _ : constant_default$6(+_), ribbon) : headRadius;
	};
	ribbon.radius = function(_) {
		return arguments.length ? (sourceRadius = targetRadius = typeof _ === "function" ? _ : constant_default$6(+_), ribbon) : sourceRadius;
	};
	ribbon.sourceRadius = function(_) {
		return arguments.length ? (sourceRadius = typeof _ === "function" ? _ : constant_default$6(+_), ribbon) : sourceRadius;
	};
	ribbon.targetRadius = function(_) {
		return arguments.length ? (targetRadius = typeof _ === "function" ? _ : constant_default$6(+_), ribbon) : targetRadius;
	};
	ribbon.startAngle = function(_) {
		return arguments.length ? (startAngle = typeof _ === "function" ? _ : constant_default$6(+_), ribbon) : startAngle;
	};
	ribbon.endAngle = function(_) {
		return arguments.length ? (endAngle = typeof _ === "function" ? _ : constant_default$6(+_), ribbon) : endAngle;
	};
	ribbon.padAngle = function(_) {
		return arguments.length ? (padAngle = typeof _ === "function" ? _ : constant_default$6(+_), ribbon) : padAngle;
	};
...
function contours_default() {
	...
	contours.thresholds = function(_) {
		return arguments.length ? (threshold = typeof _ === "function" ? _ : Array.isArray(_) ? constant_default$5(slice$1.call(_)) : constant_default$5(_), contours) : threshold;
	};
...
function density_default() {
	var x = defaultX$1, y = defaultY$1, weight = defaultWeight, dx = 960, dy = 500, r = 20, k = 2, o = r * 3, n = dx + o * 2 >> k, m = dy + o * 2 >> k, threshold = constant_default$5(20);
	function grid(data) {
	...
	density.x = function(_) {
		return arguments.length ? (x = typeof _ === "function" ? _ : constant_default$5(+_), density) : x;
	};
	density.y = function(_) {
		return arguments.length ? (y = typeof _ === "function" ? _ : constant_default$5(+_), density) : y;
	};
	...
	density.thresholds = function(_) {
		return arguments.length ? (threshold = typeof _ === "function" ? _ : Array.isArray(_) ? constant_default$5(slice$1.call(_)) : constant_default$5(_), density) : threshold;
	};
...
var epsilon$3 = 11102230246251565e-32;
var splitter = 134217729;
var resulterrbound = (3 + 8 * epsilon$3) * epsilon$3;
...
(7 + 56 * epsilon$3) * epsilon$3;
(3 + 28 * epsilon$3) * epsilon$3;
(26 + 288 * epsilon$3) * epsilon$3 * epsilon$3;
vec(4);
vec(4);
vec(4);
vec(4);
...
vec(192);
vec(192);
(10 + 96 * epsilon$3) * epsilon$3;
(4 + 48 * epsilon$3) * epsilon$3;
(44 + 576 * epsilon$3) * epsilon$3 * epsilon$3;
vec(4);
vec(4);
vec(4);
vec(4);
...
vec(1152);
vec(1152);
(16 + 224 * epsilon$3) * epsilon$3;
(5 + 72 * epsilon$3) * epsilon$3;
(71 + 1408 * epsilon$3) * epsilon$3 * epsilon$3;
vec(4);
vec(4);
vec(4);
vec(4);
...
function formatDate(date) {
	...
	return isNaN(date) ? "Invalid Date" : formatYear$1(date.getUTCFullYear(), 4) + "-" + pad$1(date.getUTCMonth() + 1, 2) + "-" + pad$1(date.getUTCDate(), 2) + (milliseconds ? "T" + pad$1(hours, 2) + ":" + pad$1(minutes, 2) + ":" + pad$1(seconds, 2) + "." + pad$1(milliseconds, 3) + "Z" : seconds ? "T" + pad$1(hours, 2) + ":" + pad$1(minutes, 2) + ":" + pad$1(seconds, 2) + "Z" : minutes || hours ? "T" + pad$1(hours, 2) + ":" + pad$1(minutes, 2) + "Z" : "");
}
...
function link_default(links) {
	var id = index$1, strength = defaultStrength, strengths, distance = constant_default$4(30), distances, nodes, count, bias, random, iterations = 1;
	if (links == null) links = [];
...
function x_default(x) {
	...
	if (typeof x !== "function") x = constant_default$4(x == null ? 0 : +x);
	function force(alpha) {
	...
	force.x = function(_) {
		return arguments.length ? (x = typeof _ === "function" ? _ : constant_default$4(+_), initialize(), force) : x;
	};
	return force;
}
//#endregion
...
function y_default(y) {
	...
	if (typeof y !== "function") y = constant_default$4(y == null ? 0 : +y);
	function force(alpha) {
	...
	force.y = function(_) {
		return arguments.length ? (y = typeof _ === "function" ? _ : constant_default$4(+_), initialize(), force) : y;
	};
	return force;
}
//#endregion
...
function locale_default(locale) {
	var group = locale.grouping === void 0 || locale.thousands === void 0 ? identity_default$3 : formatGroup_default(map$1.call(locale.grouping, Number), locale.thousands + ""), currencyPrefix = locale.currency === void 0 ? "" : locale.currency[0] + "", currencySuffix = locale.currency === void 0 ? "" : locale.currency[1] + "", decimal = locale.decimal === void 0 ? "." : locale.decimal + "", numerals = locale.numerals === void 0 ? identity_default$3 : formatNumerals_default(map$1.call(locale.numerals, String)), percent = locale.percent === void 0 ? "%" : locale.percent + "", minus = locale.minus === void 0 ? "−" : locale.minus + "", nan = locale.nan === void 0 ? "NaN" : locale.nan + "";
...
function boundsRingEnd() {
	...
	if (abs$1(deltaSum) > 1e-6) lambda0$1 = -(lambda1 = 180);
	range$1[0] = lambda0$1, range$1[1] = lambda1;
	p0 = null;
}
...
function circle_default() {
	var center = constant_default$3([0, 0]), radius = constant_default$3(90), precision = constant_default$3(2), ring, rotate, stream = { point };
	function point(x, y) {
	...
	circle.center = function(_) {
		return arguments.length ? (center = typeof _ === "function" ? _ : constant_default$3([+_[0], +_[1]]), circle) : center;
	};
...
function pointEqual_default(a, b) {
	return abs$1(a[0] - b[0]) < 1e-6 && abs$1(a[1] - b[1]) < 1e-6;
}
//#endregion
	...
	return {
		...
		point: function(lambda1, phi1) {
			...
			if (abs$1(delta - pi$1) < 1e-6) {
				...
				if (abs$1(lambda0 - sign0) < 1e-6) lambda0 -= sign0 * epsilon$1;
				if (abs$1(lambda1 - sign1) < 1e-6) lambda1 -= sign1 * epsilon$1;
...
function clipAntimeridianIntersect(lambda0, phi0, lambda1, phi1) {
	...
	return abs$1(sinLambda0Lambda1) > 1e-6 ? atan((sin$1(phi0) * (cosPhi1 = cos$1(phi1)) * sin$1(lambda1) - sin$1(phi1) * (cosPhi0 = cos$1(phi0)) * sin$1(lambda0)) / (cosPhi0 * cosPhi1 * sinLambda0Lambda1)) : (phi0 + phi1) / 2;
}
function clipAntimeridianInterpolate(from, to, direction, stream) {
	...
	if (from == null) {
		...
		stream.point(-pi$1, 0);
		stream.point(-pi$1, phi);
	} else if (abs$1(from[0] - to[0]) > 1e-6) {
...
function circle_default$1(radius) {
	...
	function intersect(a, b, two) {
		...
		var delta = lambda1 - lambda0, polar = abs$1(delta - pi$1) < epsilon$1, meridian = polar || delta < 1e-6;
		if (!polar && phi1 < phi0) z = phi0, phi0 = phi1, phi1 = z;
		if (meridian ? polar ? phi0 + phi1 > 0 ^ q[1] < (abs$1(q[0] - lambda0) < 1e-6 ? phi0 : phi1) : phi0 <= q[1] && q[1] <= phi1 : delta > pi$1 ^ (lambda0 <= q[0] && q[0] <= lambda1)) {
...
function clipRectangle(x0, y0, x1, y1) {
	...
	function corner(p, direction) {
		return abs$1(p[0] - x0) < 1e-6 ? direction > 0 ? 0 : 3 : abs$1(p[0] - x1) < 1e-6 ? direction > 0 ? 2 : 1 : abs$1(p[1] - y0) < 1e-6 ? direction > 0 ? 1 : 0 : direction > 0 ? 3 : 2;
	}
	function compareIntersection(a, b) {
...
function lengthPoint$1(lambda, phi) {
	lambda *= radians, phi *= radians;
	var sinPhi = sin$1(phi), cosPhi = cos$1(phi), delta = abs$1(lambda - lambda0), cosDelta = cos$1(delta), x = cosPhi * sin$1(delta), y = cosPhi0 * sinPhi - sinPhi0 * cosPhi * cosDelta, z = sinPhi0 * sinPhi + cosPhi0 * cosPhi * cosDelta;
...
function resampleNone(project) {
	return transformer$3({ point: function(x, y) {
		x = project(x, y);
...
function resample(project, delta2) {
	function resampleLineTo(x0, y0, lambda0, a0, b0, c0, x1, y1, lambda1, a1, b1, c1, depth, stream) {
		...
		if (d2 > 4 * delta2 && depth--) {
			var a = a0 + a1, b = b0 + b1, c = c0 + c1, m = sqrt$2(a * a + b * b + c * c), phi2 = asin$1(c /= m), lambda2 = abs$1(abs$1(c) - 1) < 1e-6 || abs$1(lambda0 - lambda1) < 1e-6 ? (lambda0 + lambda1) / 2 : atan2$1(b, a), p = project(lambda2, phi2), x2 = p[0], y2 = p[1], dx2 = x2 - x0, dy2 = y2 - y0, dz = dy * dx2 - dx * dy2;
			if (dz * dz / d2 > delta2 || abs$1((dx * dx2 + dy * dy2) / d2 - .5) > .3 || a0 * a1 + b0 * b1 + c0 * c1 < cosMinDistance) {
...
var transformRadians = transformer$3({ point: function(x, y) {
...
function conicEqualAreaRaw(y0, y1) {
	...
	if (abs$1(n) < 1e-6) return cylindricalEqualAreaRaw(y0);
...
function albersUsa_default() {
	...
	albersUsa.translate = function(_) {
		...
		alaskaPoint = alaska.translate([x - .307 * k, y + .201 * k]).clipExtent([[x - .425 * k + epsilon$1, y + .12 * k + epsilon$1], [x - .214 * k - epsilon$1, y + .234 * k - epsilon$1]]).stream(pointStream);
		hawaiiPoint = hawaii.translate([x - .205 * k, y + .212 * k]).clipExtent([[x - .214 * k + epsilon$1, y + .166 * k + epsilon$1], [x - .115 * k - epsilon$1, y + .234 * k - epsilon$1]]).stream(pointStream);
		return reset();
	};
...
function mercatorProjection(project) {
	...
	function reclip() {
		var k = pi$1 * scale(), t = m(rotation_default(m.rotate()).invert([0, 0]));
...
function conicConformalRaw(y0, y1) {
	...
	function project(x, y) {
		if (f > 0) {
			if (y < -halfPi$1 + 1e-6) y = -halfPi$1 + epsilon$1;
		} else if (y > halfPi$1 - 1e-6) y = halfPi$1 - epsilon$1;
...
function conicEquidistantRaw(y0, y1) {
	...
	if (abs$1(n) < 1e-6) return equirectangularRaw;
	function project(x, y) {
		var gy = g - y, nx = n * x;
...
function identity_default() {
	var k = 1, tx = 0, ty = 0, sx = 1, sy = 1, alpha = 0, ca, sa, x0 = null, y0, x1, y1, kx = 1, ky = 1, transform = transformer$3({ point: function(x, y) {
...
function pack_default() {
	...
	pack.padding = function(x) {
		return arguments.length ? (padding = typeof x === "function" ? x : constant_default$2(+x), pack) : padding;
	};
	return pack;
}
...
function treemap_default() {
	...
	treemap.paddingInner = function(x) {
		return arguments.length ? (paddingInner = typeof x === "function" ? x : constant_default$2(+x), treemap) : paddingInner;
	};
	treemap.paddingOuter = function(x) {
		return arguments.length ? treemap.paddingTop(x).paddingRight(x).paddingBottom(x).paddingLeft(x) : treemap.paddingTop();
	};
	treemap.paddingTop = function(x) {
		return arguments.length ? (paddingTop = typeof x === "function" ? x : constant_default$2(+x), treemap) : paddingTop;
	};
	treemap.paddingRight = function(x) {
		return arguments.length ? (paddingRight = typeof x === "function" ? x : constant_default$2(+x), treemap) : paddingRight;
	};
	treemap.paddingBottom = function(x) {
		return arguments.length ? (paddingBottom = typeof x === "function" ? x : constant_default$2(+x), treemap) : paddingBottom;
	};
	treemap.paddingLeft = function(x) {
		return arguments.length ? (paddingLeft = typeof x === "function" ? x : constant_default$2(+x), treemap) : paddingLeft;
	};
	return treemap;
}
//#endregion
...
function cornerTangents(x0, y0, x1, y1, r1, rc, cw) {
	var x01 = x0 - x1, y01 = y0 - y1, lo = (cw ? rc : -rc) / sqrt$1(x01 * x01 + y01 * y01), ox = lo * y01, oy = -lo * x01, x11 = x0 + ox, y11 = y0 + oy, x10 = x1 + ox, y10 = y1 + oy, x00 = (x11 + x10) / 2, y00 = (y11 + y10) / 2, dx = x10 - x11, dy = y10 - y11, d2 = dx * dx + dy * dy, r = r1 - rc, D = x11 * y10 - x10 * y11, d = (dy < 0 ? -1 : 1) * sqrt$1(max$1(0, r * r * d2 - D * D)), cx0 = (D * dy - dx * d) / d2, cy0 = (-D * dx - dy * d) / d2, cx1 = (D * dy + dx * d) / d2, cy1 = (-D * dx + dy * d) / d2, dx0 = cx0 - x00, dy0 = cy0 - y00, dx1 = cx1 - x00, dy1 = cy1 - y00;
...
function arc_default() {
	var innerRadius = arcInnerRadius, outerRadius = arcOuterRadius, cornerRadius = constant_default$1(0), padRadius = null, startAngle = arcStartAngle, endAngle = arcEndAngle, padAngle = arcPadAngle, context = null, path = withPath(arc);
	function arc() {
		...
		if (!(r1 > 1e-12)) context.moveTo(0, 0);
			...
			var a01 = a0, a11 = a1, a00 = a0, a10 = a1, da0 = da, da1 = da, ap = padAngle.apply(this, arguments) / 2, rp = ap > 1e-12 && (padRadius ? +padRadius.apply(this, arguments) : sqrt$1(r0 * r0 + r1 * r1)), rc = min$1(abs(r1 - r0) / 2, +cornerRadius.apply(this, arguments)), rc0 = rc, rc1 = rc, t0, t1;
			if (rp > 1e-12) {
...
function line_default(x$4, y$4) {
	var defined = constant_default$1(true), context = null, curve = linear_default, output = null, path = withPath(line);
	x$4 = typeof x$4 === "function" ? x$4 : x$4 === void 0 ? x : constant_default$1(x$4);
	y$4 = typeof y$4 === "function" ? y$4 : y$4 === void 0 ? y : constant_default$1(y$4);
	function line(data) {
		...
		for (i = 0; i <= n; ++i) {
			...
			if (defined0) output.point(+x$4(d, i, data), +y$4(d, i, data));
		}
		if (buffer) return output = null, buffer + "" || null;
	}
	line.x = function(_) {
		return arguments.length ? (x$4 = typeof _ === "function" ? _ : constant_default$1(+_), line) : x$4;
	};
	line.y = function(_) {
		return arguments.length ? (y$4 = typeof _ === "function" ? _ : constant_default$1(+_), line) : y$4;
	};
...
function area_default(x0, y0, y1) {
	...
	function area(data) {
		var i, j, k, n = (data = array_default$1(data)).length, d, defined0 = false, buffer, x0z = new Array(n), y0z = new Array(n);
		if (context == null) output = curve(buffer = path());
		for (i = 0; i <= n; ++i) {
...
function pie_default() {
	var value = identity_default$1, sortValues = descending_default$1, sort = null, startAngle = constant_default$1(0), endAngle = constant_default$1(tau), padAngle = constant_default$1(0);
	function pie(data) {
		var i, n = (data = array_default$1(data)).length, j, k, sum = 0, index = new Array(n), arcs = new Array(n), a0 = +startAngle.apply(this, arguments), da = Math.min(tau, Math.max(-tau, endAngle.apply(this, arguments) - a0)), a1, p = Math.min(Math.abs(da) / n, padAngle.apply(this, arguments)), pa = p * (da < 0 ? -1 : 1), v;
...
function link(curve) {
	let source = linkSource, target = linkTarget, x$3 = x, y$3 = y, context = null, output = null, path = withPath(link);
	function link() {
		let buffer;
		...
		if (context == null) output = curve(buffer = path());
		output.lineStart();
		argv[0] = s, output.point(+x$3.apply(this, argv), +y$3.apply(this, argv));
	...
	link.x = function(_) {
		return arguments.length ? (x$3 = typeof _ === "function" ? _ : constant_default$1(+_), link) : x$3;
	};
	link.y = function(_) {
		return arguments.length ? (y$3 = typeof _ === "function" ? _ : constant_default$1(+_), link) : y$3;
	};
...
function Symbol$1(type, size) {
	...
	type = typeof type === "function" ? type : constant_default$1(type || circle_default$2);
